...
export async function POST(request: NextRequest) {
  ...
  if (thread) {
    ...
    await notifyUser(createServiceClient(), {
      ...
      email: {
        subject: `New message from ${senderName} on ChairMatch`,
        html: `<p><strong>${senderName}</strong> sent you a message on ChairMatch:</p><p style="color:#555">${messageBody}</p><p><a href="${process.env.NEXT_PUBLIC_SITE_URL ?? ""}${thread.owner_id === authData.user.id ? `/candidate/messages/${threadId}` : `/owner/messages/${threadId}`}">Reply on ChairMatch</a></p>`,
      },
    });
  }
