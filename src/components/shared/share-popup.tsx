...
export function SharePopup({
  ...
  const shareUrl = typeof window !== "undefined" ? window.location.origin : "https://chairmatch.app";
  ...
  async function handleShare() {
    if (canNativeShare) {
      try {
        await navigator.share({ title: "ChairMatch", text: copy.body, url: shareUrl });
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-ink/40 px-4 pb-4 md:pb-0">
      <div className="w-full md:max-w-sm bg-bg-raised rounded-2xl p-5 relative">
        ...
        >
          ...
          {copied ? "Link copied" : canNativeShare ? "Share ChairMatch" : "Copy link"}
        </button>
        <button
