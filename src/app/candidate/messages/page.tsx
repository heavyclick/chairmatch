...
interface ThreadSummary {
  id: string;
  owner_id: string;
  owner?: { practice_name: string; photo_url: string | null };
      ...
      {!loading && threads.length === 0 && (
        <div className="rounded-xl border border-dashed border-line p-10 text-center">
          ...
          <p className="text-[13px] text-ink-faint">
            When a practice reaches out, you&apos;ll see it here.
          </p>
        </div>
      )}
            ...
            <div className="relative shrink-0">
              <div className="w-11 h-11 rounded-full bg-gradient-to-br from-teal to-teal-deep flex items-center justify-center text-white font-serif font-semibold overflow-hidden">
                {t.owner?.photo_url ? (
                  ...
                  <img src={t.owner.photo_url} alt="" className="w-full h-full object-cover" />
                ) : (
                  t.owner?.practice_name?.[0] ?? "?"
                )}
              </div>
              {t.is_unread && (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                <span className={`text-[14.5px] truncate ${t.is_unread ? "font-bold" : "font-semibold"}`}>
                  {t.owner?.practice_name ?? "Practice"}
                </span>
