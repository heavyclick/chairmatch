  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      const { data: candidate } = await supabase
        ...
        .eq("id", data.user.id)
        .maybeSingle();
      setPhotoUrl(candidate?.photo_url ?? null);
    });
  }, []);
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside className="hidden md:flex md:w-[210px] md:flex-col md:shrink-0 bg-ink px-4 py-7 sticky top-0 h-screen overflow-y-auto">
        {/* Points into the app now, not out to the marketing homepage --
            this layout only ever renders for an already-authenticated
            candidate. */}
        <Link href="/candidate/dashboard" className="flex items-center gap-2 px-2 mb-9">
          <span className="w-[7px] h-[7px] rounded-full bg-coral" />
          <span className="font-serif text-lg font-semibold text-white">Hdenta</span>
        </Link>
        <CandidateSidebarNav />
      ...
      <header className="md:hidden fixed top-0 inset-x-0 z-30 flex items-center justify-between bg-ink px-5 py-3.5">
        <Link href="/candidate/dashboard" className="flex items-center gap-2">
          <span className="w-[7px] h-[7px] rounded-full bg-coral" />
          <span className="font-serif text-base font-semibold text-white">Hdenta</span>
        </Link>
        <div className="flex items-center gap-2.5">
          <NotificationBell dark />
          <ProfileMenu profileHref="/candidate/profile" photoUrl={photoUrl} dark />
        </div>
      </header>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <div className="hidden md:flex items-center justify-end gap-2.5 px-8 py-4 sticky top-0 z-20 bg-bg/80 backdrop-blur-sm">
          ...
          <NotificationBell />
          <ProfileMenu profileHref="/candidate/profile" photoUrl={photoUrl} />
        </div>
